...
landmark models and the attributed static alphabet model. The pipeline accepts
...
- Face tracking is optional and adds latency. Expression labels are not mapped
  to inferred emotional state or automatic speech.

Expo's goose, design tokens, caption correction, transcript, voice consent and
server proxy remain. Only explicit confirmed text reaches optional voice.
