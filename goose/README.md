...
not make it server-only. Use only disposable development credentials on trusted
devices; before distributing the app, replace direct calls with an authenticated
backend proxy. No `.env` or API credentials are included in this import.
...
Expo embeds the key in the client bundle; it does **not** stay on this machine.
Do not commit `.env` or ship this client-side key in a shared build. When real
ASR exists, it should pass its English string into `speakEnglish` instead of the
text field, after provider authentication has moved to a backend.
